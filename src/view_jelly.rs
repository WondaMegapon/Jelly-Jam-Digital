...
pub async fn load_card_jelly() -> Vec<Texture2D> {
    let mut jelly_textures = Vec::new();

    let paths = fs::read_dir("./assets/cards/jelly/").unwrap();
...
pub fn draw_view_jelly(jelly_textures: &[Texture2D], state: &mut GameState) {
    ...
    let total_cards = jelly_textures.len();
    ...
    for (i, texture) in jelly_textures.iter().enumerate() {
        let col = i % cols;
        let row = i / cols;
