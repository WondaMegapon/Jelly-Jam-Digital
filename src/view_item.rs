...
pub async fn load_card_item() -> Vec<Texture2D> {
    let mut item_textures = Vec::new();

    let paths = fs::read_dir("./assets/cards/item/").unwrap();
...
pub fn draw_view_item(item_textures: &[Texture2D], state: &mut GameState) {
    ...
    let total_cards = item_textures.len();
    ...
    for (i, texture) in item_textures.iter().enumerate() {
        let col = i % cols;
        let row = i / cols;
