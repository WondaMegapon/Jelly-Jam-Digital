...
pub async fn load_card_mutation() -> Vec<Texture2D> {
    let mut mutation_textures = Vec::new();

    let paths = fs::read_dir("./assets/cards/mutation/").unwrap();
    ...
    for path in paths {
        ...
        if path.extension().unwrap_or_default() == "png" {
            ...
            mutation_textures.push(texture);
        }
    }
...
pub fn draw_view_mutation(mutation_textures: &[Texture2D], state: &mut GameState) {
    ...
    let total_cards = mutation_textures.len();
    ...
    for (i, texture) in mutation_textures.iter().enumerate() {
        let col = i % cols;
        let row = i / cols;
